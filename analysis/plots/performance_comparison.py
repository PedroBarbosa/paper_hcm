...
def plot_metrics(data, fname):
  ...
  #  plt.hlines(y=my_range, xmin=data['specificity'], xmax=data['sensitivity'], color='grey', alpha=0.75)
    plt.scatter(data['specificity'], my_range, color='skyblue', alpha=1, marker='s', edgecolors='black', linewidths=0.5, label='Specificity')
    plt.scatter(data['sensitivity'], my_range, color='yellow', alpha=0.75, marker='^', edgecolors='black',linewidths=0.5, label='Sensitivity')
    ...
    for idx, rows in data.iterrows():
        ...
        height = 1
        ax.add_patch(plt.Rectangle(xy=( width[0] - 0.01, my_range[i] - 0.4), width=width[1] - width[0] + 0.02, height=height, linewidth=0.75, color='gray',fill=False))
        i+=1
...
def plot_tools_by_type(data,fname):
    ...
    fig, ax = plt.subplots()
    plt.xlim(df['total'][0])
    ax.barh(ind, df['correct'], align='center', color='green', zorder=10, alpha=0.8, height=w, edgecolor="black", linewidth=0.75)
...
def plot_tools(data, fname):
    ...
    axes[0].barh(ind, df['tp'], align='center', color='green', alpha=0.8, zorder=10, height=w, edgecolor='black',linewidth=0.75)
    axes[0].barh(ind, df['fn'], left=correct_p, align='center', alpha=0.8, color='darkred', zorder=10, height=w, edgecolor='black',linewidth=0.75)
    axes[0].barh(ind, df['mp'], left=incorrect_p, align='center', color='lightgrey', zorder=10, height=w, edgecolor='black',linewidth=0.75)
    axes[1].barh(ind, df['tn'], align='center', color='green', alpha=0.8, zorder=10, height=w, edgecolor='black',linewidth=0.75)
    axes[1].barh(ind, df['fp'], left=correct_n, align='center', alpha=0.8,color='darkred', zorder=10, height=w, edgecolor='black',linewidth=0.75)
    axes[1].barh(ind, df['mn'], left=incorrect_n, align='center',  color='lightgrey', zorder=10, height=w, edgecolor='black',linewidth=0.75)
...
def generate_performance_comparison(dataset, filters, thresholds, name, folder, new_thresholds=None):
    for filtername, filterfunction in filters:
        statistics = defaultdict(list)

        df = filterfunction(dataset).copy()
        if df.shape[0] < 10:
            print("{} has not a minimum number of {} variants (10) to evaluate tools performance. ({})".format(name,
                                                                                                          filtername, df.shape[0]))
            continue
        ...
        for tool, *args in thresholds:
            ...
            try:
                ...
                statistics['weighted_f1'].append(round((2 * (precision * recall) / (precision + recall)) * coverage,2))
        ...
        plot_tools(stats_df, os.path.join(folder, "figures", 'tools_analysis_' + name + "_" + filtername + suffix))
        plot_tools_paper(stats_df,os.path.join(folder, "figures", 'tools_analysis_paper_' + name + "_" + filtername + suffix))
        plot_metrics(stats_df,os.path.join(folder,"figures", 'tools_performance_metrics_' + name + "_" + filtername + suffix))

        stats_df.drop(['filter'], axis=1).to_csv(os.path.join(folder,"statistics_{}.csv").format(filtername),sep="\t", index=False)
        if "weighted_accuracy" in list(stats_df):
            stats_df[["tool","weighted_accuracy", "accuracy","weighted_f1", "f1"]].sort_values(["weighted_accuracy"], ascending=False).to_csv(
                os.path.join(folder,"tools_ranking_{}.csv").format(filtername),sep="\t", index=False)
        else:
            stats_df[["tool", "weighted_f1", "f1", "accuracy"]].sort_values(["weighted_f1"],ascending=False).to_csv(
                os.path.join(folder, "tools_ranking_{}.csv").format(filtername), sep="\t", index=False)
